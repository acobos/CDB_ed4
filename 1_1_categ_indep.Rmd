...
## Low birth weight (LBW) related to smoking?
...
Evidence of a different probability of LBW in smokers and non-smokers.
...
No evidence of a different probability of LBW in HT and non-HT.
...
This *is not* the table we need!  Let's transpose it with function `t()`...
...
```{r}
bw_smoke %>% t()                 # same, using the pipe
```
...
```{r}
res$measures[-3,]                  # getting all rows but the second (and all cols)
res$measures[1,]  %>% round(2)     # getting the 1st row only (all cols), rounded 
res$measures[1,1] %>% round(2)     # getting the 1st row, 1st col, rounded 
