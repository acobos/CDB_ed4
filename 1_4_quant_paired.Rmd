...
Difference of pre & pos means = mean of individual differences

```{r}
sapply(d, mean) %>% round(1)                     # compute mean of all vars in d
```



``r paste0(round(mean(d$pos),1), " - ", round(mean(d$pre),1), " = ", round(mean(d$pos) - mean(d$pre),1))``
...
- approximate symmetry (i.e., *slight* asymmetry)

- a single outlier but not influential (mean $\approx$ median)
...
Murder rates (per 100,000 inhabitants) by gender in 198 countries
