...
1. Describe variables in dataframe `d`.  

1. Produce boxplots of:
    - `fev` by `smoke`, and 
    - `age` by `smoke`. 

1. Fit a model for `fev` with `smoke` as predictor; what is the effect of `smoke` on `fev`?

1. Fit a second model by adding `age` as predictor; what is now the effect of `smoke` on `fev`? 

1. Do you think `age` is a confounder for `smoke`?


## Exercise 2 (Recap)

1. Subset non-smokers into a new dataframe to conduct the following analysis.

1. Fit a model for `fev` with `height` and `gender` as predictors:
    - get a summary of the model and interpret the model coefficients.
    - plot the data along with the model.
    - assess model assumptions graphically; is there any non-linearity?  
    - does a polynomial for `height` improve the fit?  
    - re-assess model assumptions; is there any remaining problem?
    - fit a model with `log(fev)` as the dependent variable. 
